...
from forms import RegistrationForm, LoginForm, ContactForm, ReplyForm
...
def profile():
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM customer WHERE username = %s", (g.user,))
    user = cur.fetchone()
...
@app.route("/manager")
def manager():
    return render_template("manager/manager_account.html", queries=queries)
